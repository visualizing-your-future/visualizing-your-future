...
const WP2503PageEmail = () => {
  const email = useParams()._email;
  const worksheetName = useParams()._worksheetName;
  const { ready, wp2503 } = useTracker(() => {
    const subscription = WP2503.subscribeWP2503Admin();
    const rdy = subscription.ready();
    const wp2503Data = WP2503.find({ owner: email, worksheetName: worksheetName }, { sort: { year: 1 } }).fetch();
    console.log(wp2503Data);
    return {
      wp2503: wp2503Data,
      ready: rdy,
    };
  ...
  const years = wp2503.map(entry => entry.year).slice(0);
...
export default WP2503PageEmail;
