...
export const bossAccountantProfilePublications = {
  bossAccountantProfile: 'BossAccountantProfile',
  bossAccountantProfileAdmin: 'BossAccountantProfileAdmin',
};

class BossAccountantProfileCollection extends BaseProfileCollection {
  constructor() {
    super('BossAccountantProfile', new SimpleSchema({}));
  }
  ...
  define({ email, firstName, lastName, password, clients }) {
    if (Meteor.isServer) {
      ...
      if (!user) {
        const role = ROLE.BOSSACCOUNTANT;
  ...
  publish() {
    if (Meteor.isServer) {
      ...
      Meteor.publish(bossAccountantProfilePublications.bossAccountantProfile, function publish() {
        if (this.userId) {
      ...
      Meteor.publish(bossAccountantProfilePublications.bossAccountantProfileAdmin, function publish() {
   ...
   */
  subscribeBossAccountantProfilesUser() {
    if (Meteor.isClient) {
      return Meteor.subscribe(bossAccountantProfilePublications.bossAccountantProfile);
    }
    return null;
  }
   ...
   * It subscribes to the entire collection.
   */
  subscribeBossAccountantProfilesAdmin() {
    if (Meteor.isClient) {
      return Meteor.subscribe(bossAccountantProfilePublications.bossAccountantProfileAdmin);
    }
    return null;
  }
  ...
  assertValidRoleForMethod(userId) {
    this.assertRole(userId, [ROLE.ADMIN, ROLE.BOSSACCOUNTANT]);
  }
  ...
  checkIntegrity() {
    ...
    this.find().forEach((doc) => {
      if (doc.role !== ROLE.BOSSACCOUNTANT) {
        problems.push(`BossAccountantProfile instance does not have ROLE.BOSSACCOUNTANT: ${doc}`);
      }
    });
    return problems;
  }
 ...
 * @type {BossAccountantProfileCollection}
 */
export const BossAccountantProfiles = new BossAccountantProfileCollection();
